...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "programma" }, _attrs))}><div class="container"><div class="days"${ssrRenderAttr("id", $props.dag)}><!--[-->`);
  ssrRenderList($data.vandaag, (item, index) => {
    _push(`<div style="${ssrRenderStyle({ height: `${Math.max(($options.timeToPx(item.end) - $options.timeToPx(item.start)) * $options.grow, 40)}px`, top: `${$options.timeToPx(item.start) * $options.grow}px` })}" class="${ssrRenderClass([{ "leftevent": item.lokatie === "links", "rightevent": item.lokatie === "rechts" }, "event"])}"><div class="textcontainer"><span class="title fitTitle">${ssrInterpolate(item.titel[_ctx.language])}</span><br>`);
    if (!!item.neptijd) {
      _push(`<span class="time fitTime">${ssrInterpolate(`${item.nepstart} - ${item.nepend}`)}</span>`);
    } else {
      _push(`<span class="time fitTime">${ssrInterpolate(`${Math.floor(item.start / 100)}:${$options.pad(item.start % 100, 2)} - ${Math.floor(item.end / 100)}:${$options.pad(item.end % 100, 2)}`)}</span>`);
    }
    _push(`</div></div>`);
  });
  _push(`<!--]--></div></div><div class="${ssrRenderClass([{ "is-active": $data.modal }, "modal"])}"><div class="modal-background"></div><div class="modal-content"><div class="modaltitle">${ssrInterpolate($data.modalItem.titel[_ctx.language])}</div>`);
  if (!!$data.modalItem.neptijd) {
    _push(`<span class="time">${ssrInterpolate(`${$data.modalItem.nepstart} - ${$data.modalItem.nepend}`)}</span>`);
  } else {
    _push(`<span class="time">${ssrInterpolate(`${Math.floor($data.modalItem.start / 100)}:${$options.pad($data.modalItem.start % 100, 2)} - ${Math.floor($data.modalItem.end / 100)}:${$options.pad($data.modalItem.end % 100, 2)}`)}</span>`);
  }
  _push(`<br><p>${$data.modalItem.beschrijving[_ctx.language]}</p></div><button class="modal-close is-large"></button></div></div>`);
}
...
const programmas = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["ssrRender", _sfc_ssrRender]]);
const __default__ = {
  data() {
    return {
      dagen: {
...
const _sfc_main = /* @__PURE__ */ Object.assign(__default__, {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="columns"><div class="column"><div class="columns"><div class="column"><div class="${ssrRenderClass([{ "mobiledagcontainerparent": _ctx.$mq === "mobile" }, "dagcontainer"])}"><h3>${ssrInterpolate(_ctx.dagen[unref(language)][0])}</h3>`);
      _push(ssrRenderComponent(programmas, {
        dag: "maandag",
        class: { "mobiledagcontainer": _ctx.$mq === "mobile" }
      }, null, _parent));
      _push(`</div></div><div class="column"><div class="${ssrRenderClass([{ "mobiledagcontainerparent": _ctx.$mq === "mobile" }, "dagcontainer"])}"><h3>${ssrInterpolate(_ctx.dagen[unref(language)][1])}</h3>`);
      _push(ssrRenderComponent(programmas, {
        dag: "dinsdag",
        class: { "mobiledagcontainer": _ctx.$mq === "mobile" }
      }, null, _parent));
      _push(`</div></div><div class="column"><div class="${ssrRenderClass([{ "mobiledagcontainerparent": _ctx.$mq === "mobile" }, "dagcontainer"])}"><h3>${ssrInterpolate(_ctx.dagen[unref(language)][2])}</h3>`);
      _push(ssrRenderComponent(programmas, {
        dag: "woensdag",
        class: { "mobiledagcontainer": _ctx.$mq === "mobile" }
      }, null, _parent));
      _push(`</div></div><div class="column"><div class="${ssrRenderClass([{ "mobiledagcontainerparent": _ctx.$mq === "mobile" }, "dagcontainer"])}"><h3>${ssrInterpolate(_ctx.dagen[unref(language)][3])}</h3>`);
      _push(ssrRenderComponent(programmas, {
        dag: "donderdag",
        class: { "mobiledagcontainer": _ctx.$mq === "mobile" }
      }, null, _parent));
...
//# sourceMappingURL=schema-87e89be2.mjs.map
